...
function removeFirstOccurrences(str, value) {
  const substringIndex = str.indexOf(value);

  const result =
    substringIndex !== -1
      ? str.slice(0, substringIndex) + str.slice(substringIndex + value.length)
      : str;
...
function removeLastOccurrences(str, value) {
  const substringLastIndex = str.lastIndexOf(value);

  const result =
    substringLastIndex !== -1
      ? str.slice(0, substringLastIndex) +
        str.slice(substringLastIndex + value.length)
      : str;
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (vowels.includes(str[i].toLowerCase())) sum += 1;
  }
...
function isPalindrome(str) {
  ...
  const result =
    ...
    reverseStr.toLowerCase().replace(/[.,!?\s]/g, '');
...
function findLongestWord(sentence) {
  let longestWord = '';
  let longestLength = 0;
  const sentenceWordsArray = sentence.split(' ');

  for (let i = 0; i < sentenceWordsArray.length; i += 1) {
    if (sentenceWordsArray[i].length > longestLength) {
      longestWord = sentenceWordsArray[i];
      longestLength = sentenceWordsArray[i].length;
    }
  }
...
function reverseWords(str) {
  const sentenceWordsArray = str.split(' ');
  const resultArray = [];

  for (let i = 0; i < sentenceWordsArray.length; i += 1) {
    resultArray.push(sentenceWordsArray[i].split('').reverse().join(''));
  }
...
function invertCase(str) {
  let result = '';
  const sentenceCharsArray = str.split('');

  for (let i = 0; i < sentenceCharsArray.length; i += 1) {
    if (sentenceCharsArray[i] === sentenceCharsArray[i].toLowerCase()) {
      result += sentenceCharsArray[i].toUpperCase();
    } else {
      result += sentenceCharsArray[i].toLowerCase();
    }
  }
